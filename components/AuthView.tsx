...
export function AuthView() {
  const { authScreen, setAuthScreen, login, register, hasCompletedOnboarding, setActiveTab } = useApp();
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await register(regName, regEmail, regPassword, regUsername);
      setIsLoading(false);
      if (!res.success) {
        setErrorMessage(res.error || 'Pendaftaran akun gagal.');
      }
    } catch (err: any) {
      setIsLoading(false);
      setErrorMessage(err?.message || 'Terjadi gangguan saat mendaftar.');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 selection:bg-teal-100 selection:text-teal-900">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-1 text-center text-xs sm:text-sm text-slate-500 max-w-xs mx-auto">
          {authScreen === 'login'
            ? 'Masuk ke sistem kasir, stok & pembukuan warung'
            : 'Daftar akun pemilik baru dan mulai kelola usaha Anda'}
        </p>
      </div>
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md">
              ...
              <div>
                ...
                <div className="relative rounded-xl">
                  ...
                  <input
                    ...
                    placeholder="Contoh: staging atau kasir1"
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-3.5">
              ...
              <div>
                ...
                <div className="relative rounded-xl">
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 font-mono placeholder-slate-400 focus:outline-none focus:border-teal-500 focus:bg-white transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-teal-500 focus:bg-white transition-colors"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-2 py-3 px-4 rounded-xl text-sm font-semibold text-white bg-teal-600 hover:bg-teal-700 active:bg-teal-800 shadow-md shadow-teal-600/20 active:scale-[0.99] transition-all cursor-pointer disabled:opacity-50"
                >
                  <span>{isLoading ? 'Mendaftarkan Akun...' : 'Daftar & Lanjut ke Onboarding'}</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>
        ...
        <p className="mt-4 text-center text-xs text-slate-400 flex items-center justify-center gap-1.5">
          ...
          <span>Autentikasi Aman & Isolasi Data Akun Terproteksi RLS</span>
        </p>
      </div>
    </div>
  );
